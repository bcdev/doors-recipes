...
RUN echo "xcube version: v1.9.0"; \
    ...
    echo "kassandra reference store version: 0.1.3}";
...
RUN micromamba install -y -n base -c conda-forge xcube=1.9.0 xvec pip "botocore<1.36" pydap

RUN micromamba run -n base python3 -m pip install --no-deps https://github.com/xcube-dev/xcube-vdc-places/archive/v0.1.tar.gz

RUN micromamba run -n base python3 -m pip install --no-deps https://github.com/bcdev/doors-stores/archive/v0.1.3.tar.gz
